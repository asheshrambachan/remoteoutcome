...
Because the same estimator and resampling procedure are used, the treatment effect estimates here are **identical** to those in *Rambachan, Singh, and Viviano (2025)*. There is a minimal variation in confidence intervals that arise from the updates in bootstrap procedure, not from the underlying estimator or data. Specifically, the paper computed the bootstrap standard errors using the `boot` package with parallelization. We now use a *custom cluster bootstrap* that supports deterministic seeding via `fix_seed = TRUE`, setting `set.seed(b)` for the *b*‑th replication. This preserves the resampling logic, ensuring reproducibility even under parallel execution.
...
This function implements the RSV estimator described in **Rambachan, Singh, and Viviano (2025)** (Algorithm 1).

In this vignette, instead of retraining predictive models, we use pre-computed predictions generated with the `randomForest` package--the same method used to produce the estimates reported in the paper. This approach allows the results here to closely mirror the published values. 
There are minor numerical differences in CI due to changes in bootstrap procedure as mentioned previously.
