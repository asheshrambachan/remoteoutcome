...
This vignette demonstrates how to use the `remoteoutcome` package to estimate treatment effects when outcomes are measured using remotely sensed variables (RSVs). Specifically, it replicates **Figure 7**  of *Rambachan, Singh, and Viviano (2025)*, which analyzes data from a randomized evaluation of an anti-poverty program in Andhra Pradesh, India (Muralidharan et al., 2017).

This vignette reproduces estimates that are **numerically identical** to those in *Rambachan, Singh, and Viviano (2025)*, with only minor differences arising from implementation updates that do **not** affect the estimator or any of its statistical properties. The updates are as follows:
...
For completeness, we also provide a complementary vignette, `vignette("poverty-application-preds")`, which reproduces the figures using the original `randomForest` predictions. These plots align even more closely with those in the published paper, with only small differences in standard errors due to the updated bootstrap procedure described above.
...
- `ml_params`: Random forest parameters:
  ...
  - `cores`: Number of cores used by `ranger` for parallel training.
...
rsv_real_Ycons <- rsv_estimate(
  ...
  eps = 1e-2,
  method = "none",
  ml_params = list(ntree = 100, classwt_Y = c(10, 1), seed=42, cores = cores),
  se = TRUE,
  se_params = list(B = 1000, fix_seed = TRUE, cores = cores, clusters = clusters),
)
...
obs_clusters <- unique(data$clusters)[1:floor(length(unique(data$clusters)) / 2)]
