...
This vignette demonstrates how to use the `remoteoutcome` package to estimate treatment effects when outcomes are measured using remotely sensed variables (RSVs). It replicates **Figure 7**  of *Rambachan, Singh, and Viviano (2025)*, which analyzes data from a randomized evaluation of an anti-poverty program in Andhra Pradesh, India (Muralidharan et al., 2016; Muralidharan et al. 2023). 
...
2. The paper computed the bootstrap standard errors using the `boot` package with parallelization without setting the seed. `remoteoutcome` uses a custom cluster bootstrap that supports deterministic seeding via `fix_seed = TRUE`, setting `set.seed(b)` for the *b*‑th replication.

Under these changes, both point estimates and confidence intervals remain numerically identical to those in the paper. For completeness, we also provide a complementary vignette, `vignette("poverty-application-original-preds")`, which reproduces the figures using the original `randomForest` predictions. These plots align even more closely with those in the published paper, with only small differences in standard errors due to the updated bootstrap procedure described above.
